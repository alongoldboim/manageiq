...
begin
  ...
  result = $evm.root['container_deployment'].run_playbook_command($evm.root['deployment_master'], $evm.root['ssh_username'], "atomic-openshift-installer -u -c /usr/share/ansible/openshift-ansible/inventory.yaml install 1> /tmp/openshift-ansible.log.1 2> /tmp/openshift-ansible.log.2 < /dev/null")
  if result[:finished]
