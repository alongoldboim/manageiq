...
begin
  ...
  create_ansible_inventory_file(true) if $evm.root['container_deployment'].perform_agent_commands($evm.root['deployment_master'], $evm.get_state_var(:ssh_user), ["sudo cat /etc/redhat-release"])[:stdout].include?("Red Hat Enterprise Linux")
rescue => err
  $evm.log(:error, "[#{err}]\n#{err.backtrace.join("\n")}")
