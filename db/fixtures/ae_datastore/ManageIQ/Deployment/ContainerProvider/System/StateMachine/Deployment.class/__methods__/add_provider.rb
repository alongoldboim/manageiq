...
def provider_token
  $evm.root['container_deployment'].perform_agent_commands($evm.root['deployment_master'], $evm.root['ssh_username'], ["oc get secrets `oc describe serviceaccount management-admin -n management-infra | awk '/Tokens:/ { print $2 }'` --template '{{.data.token}}' -n management-infra | base64 -d"])[:stdout]
end
