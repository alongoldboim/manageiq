...
def provider_token
  $evm.root['container_deployment'].perform_agent_commands($evm.root['deployment_master'], $evm.get_state_var(:ssh_user), ["sudo oc get secrets `oc describe serviceaccount management-admin -n management-infra | awk '/Tokens:/ { print $2 }'` --template '{{.data.token}}' -n management-infra | base64 -d"])[:stdout]
end
...
def add_provider
  ...
  new_provider = $evm.root['container_deployment'].add_deployment_provider(
    ...
    :provider_port      => "8443",
    :provider_hostname  => $evm.root['container_deployment'].perform_agent_commands($evm.root['deployment_master'], $evm.get_state_var(:ssh_user), ["sudo echo $HOSTNAME"])[:stdout].gsub("\r\n", ''),
