  ...
  class CaptureContext
    ...
    def process_strings_data(keys)
      keys.each do |resource|
        begin
          save_date_as_custom_attribute(resource.split("/").last, hawkular_client.strings.get_data(resource,:starts => (@start_time - @interval).to_i.in_milliseconds, :order => "DESC").first)
