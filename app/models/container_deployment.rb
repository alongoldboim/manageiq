  ...
  def addresses_array(container_deployment_nodes)
    ...
    container_deployment_nodes.each do |container_deployment_node|
      ip = if container_deployment_node.vm
             container_deployment_node.vm.hardware.ipaddresses.last unless container_deployment_node.tags.any? { |tag| tag.name.include?("deployment_master") }
           elsif container_deployment_node.address
             container_deployment_node.address unless container_deployment_node.tags.any? { |tag| tag.name.include?("deployment_master") }
           end
      ip_array << ip if ip
    end
    ip_array
...
EOS
  ...
  def generate_roles
    ...
    unless identity_provider_auth.first.htpassd_users.empty?
      result["master"]["openshift_master_htpasswd_users"] = identity_provider_auth.first.htpassd_users
    end
