...
eos
  ...
  def create_deployment_auth(options)
    self.deployment_authentication << DeploymentAuthentication.new(:name                           => options[:auth_name],
  ...
  def create_deployment_nodes(vm_groups, labels, tags, vm_id = nil, ip_or_hostname = nil)
    vm_groups.each_with_index do |vms, index|
      vms.each do |vm_attr|
        ...
        container_deployment_node.labels = JSON.parse(labels[vm_attr].to_json).to_h if labels[vm_attr]
