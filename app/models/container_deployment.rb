class ContainerDeployment < ApplicationRecord
  ...
  def addresses_array(container_deployment_nodes)
    ...
    container_deployment_nodes.each do |container_deployment_node|
      ip = if container_deployment_node.vm
              container_deployment_node.vm.hardware.ipaddresses.last unless (container_deployment_node.tags.any? do |tag| tag.name.include?("deployment_master") end)
            elsif container_deployment_node.address
              container_deployment_node.address unless (container_deployment_node.tags.any? do |tag| tag.name.include?("deployment_master") end)
            end
      ip_array << ip if ip
    end
    ip_array
...
[OSEv3:children]
nodes
#{"masters" unless (roles_addresses("master") - [roles_addresses("deployment_master")]).empty?}
...
[masters]
#{(roles_addresses("deployment_master").to_s + " ansible_connection=local ansible_sudo=no openshift_scheduleable=True")}
#{roles_addresses("master").join("\n") unless (roles_addresses("master")).empty?}
[nodes]
#{(roles_addresses("deployment_master").to_s) if (container_nodes_by_role("deployment_master").first.tags.any? do |tag| tag.name.include?("node") end)}
