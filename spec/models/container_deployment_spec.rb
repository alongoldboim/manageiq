describe ContainerDeployment do
  before(:each) do
    @container_deployment = FactoryGirl.create(:container_deployment, :method_type => "non_managed", :version => "v3")
    ...
    @container_deployment.create_deployment_authentication("userid" => "root", "auth_key" => "-----BEGIN RSA PRIVATE KEY----- exmaple -----END RSA PRIVATE KEY-----", "public_key" => "public_key", "type" => "AuthPrivateKey")
  end
  ...
  context "container deployment nodes" do
    ...
    it "create deployment nodes works properly" do
      ...
      @container_deployment.create_deployment_nodes([{"name" => "10.0.0.2", "roles" => {"node" => true}}, {"name" => "37.142.68.50", "roles" => {"node" => true}}, {"name" => "example.com", "roles" => {"master" => true, "deployment_master" => true}}])
